...
namespace Aptos.Rest
{
    ...
    public class AptosTokenClient
    {
        ...
        public async Task<ReadObject?> ReadObject(AccountAddress address)
        {

            var resourcesResp = await Client.GetAccountResourcesAsync(address);
            ...
            var readResources = JsonConvert.DeserializeObject<List<IResourceBase>>(resourcesResp, new ResourceBaseListConverter<IResourceBase>());
            ...
            return new ReadObject(resources);
        }
            ...
            )
        {
            ...
            EntryFunction payload = EntryFunction.Natural(
                ...
                "create_collection",
                new TagSequence(Array.Empty<ISerializableTag>()),
                new Sequence(transactionArguments)
            );

            var signedTransaction = await Client.CreateBCSSignedTransactionAsync(Creator, new BCS.TransactionPayload(payload));

            if (signedTransaction == null)
                return null;

            return await Client.SubmitBCSTransactionAsync(signedTransaction);
        ...
        public async Task<string?> IMintSoulBoundToken(
            Account Creator,
            string Collection,
            string Description,
            string Name,
        ...
        {
            return await Client.TransferObjectAsync(Owner, Token, To);
        }

        public async Task<string?> BurnToken(
            Account Creator,
            AccountAddress Token
        )
        {
        ...
        public async Task<string?> FreezeToken(
            Account Creator,
            AccountAddress Token
        )
        {
        ...
        public async Task<string?> UnfreezeToken(
            Account Creator,
            AccountAddress Token
        )
        {
        ...
        public async Task<string?> AddTokenProperty(
            Account Creator,
            AccountAddress Token,
            Property Prop)
        {
        ...
        public async Task<string?> RemoveTokenProperty(
            Account Creator,
            AccountAddress Token,
            string Name)
        {
        ...
        public async Task<string?> UpdateTokenProperty(
            Account Creator,
            AccountAddress Token,
            Property Prop)
        {
        ...
        public async Task<List<AccountAddress>?> TokensMintedFromTransaction(string TransactionHash)
        {
            var responseTx = await Client.TransactionByHashAsync(TransactionHash);
            ...
            var mints = new List<AccountAddress>();

            foreach (var txEvent in responseTx.Events)
            {
